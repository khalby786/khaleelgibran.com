    ...
    <defs>
      ...
      >
        ...
        <feBlend
          ...
          result="effect1_dropShadow_2_333"
        />
        <feBlend
          mode="normal"
          in="SourceGraphic"
          in2="effect1_dropShadow_2_333"
          result="shape"
        />
      </filter>
    </defs>
...
export default {
  ...
  methods: {
    animateCursor(point) {
      if (!this.$refs.rCursor) return;

      this.$refs.rCursor.style.transform = `translate(${point[0]}px, ${point[1]}px)`;
    },
    usePerfectCursor(callback, point) {
      if (!this.pc) {
        this.pc = new PerfectCursor(callback);
      }

      if (point) {
        this.pc.addPoint(point);
        this.pc.dispose();
      }
    ...
    onPointMove(point) {
      this.usePerfectCursor(this.animateCursor.bind(this), point);
    },
  },
  watch: {
    point: {
      handler(newPoint, oldPoint) {
        this.onPointMove(newPoint);
      },
      immediate: true,
    },
  },
